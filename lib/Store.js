...
class Receipt{
	...
	toJson(){
		...
		this.storeOffers.forEach((offer)=>{
			...
			if(this.offerIsApplicable(offer)){
	...
	offerIsApplicable(offer){

		let offerItem = offer.item;
		let minItems = offer.getAmount;

		let isApplicable = false;
		let instances = 0;

		this.shoppingItems.forEach((item)=>{
			if(offerItem == item.constructor.name){
				isApplicable = true;
				instances++;
			}
		}, this);

		if(instances < minItems){
			isApplicable = false;
		}
	...
	print(){
		...
		this.storeOffers.forEach((offer)=>{
			...
			//find if this offer is applicable to any item
			if(this.offerIsApplicable(offer) == true){
				let saving = offer.calculate(this.shoppingItems);
				totalSaving += saving;
